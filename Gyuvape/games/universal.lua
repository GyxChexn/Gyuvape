...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/GyxChexn/Gyuvape/'..readfile('Gyuvape/profiles/commit.txt')..'/'..select(1, path:gsub('newvape/', '')), true)
		end)
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('Gyuvape/assets/new/blur.png')
	...
	ChatSpammer = vape.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				local ind = 1
				repeat
					local message = (#Lines.ListEnabled > 0 and Lines.ListEnabled[math.random(1, #Lines.ListEnabled)] or 'GyuVapeV4 on top')
